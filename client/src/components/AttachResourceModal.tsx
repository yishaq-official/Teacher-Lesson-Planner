  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="glass-panel rounded-2xl border border-slate-700/60 w-full max-w-3xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Search & Filter Bar */}
        <div className="p-3 sm:p-4 border-b border-slate-800/80 bg-slate-900/40 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700/80 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-200 focus:outline-none focus:border-orange-500"
            />
          </div>
          <select
            value={subjectFilter}
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded bg-orange-500/10 text-orange-400 border border-orange-500/20">
                        {item.type}
                      </span>
        ...
        {/* Modal Footer */}
        <div className="px-4 sm:px-6 py-3 border-t border-slate-800 bg-slate-950/60 flex items-center justify-between text-xs text-slate-400">
          <span>{attachedResources.length} resource(s) selected</span>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl gradient-bg-primary text-white font-semibold transition-colors"
          >
            Done
          </button>
        </div>
